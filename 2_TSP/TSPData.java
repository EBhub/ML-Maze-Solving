...
/**
 * Class containing the product distances. Can be either build from a maze, a
 * product location list and a PathSpecification or be reloaded from a file.
 */
public class TSPData implements Serializable {
    ...
    /**
     * Calculate the routes from the product locations to each other, the start, and
     * the end. Additionally generate arrays that contain the length of all the
     * routes.
     * 
     * @param maze
     */
    ...
    public boolean equals(Object other) {
        if (!(other instanceof TSPData)) {
            ...
            return Arrays.deepEquals(this.distances, otherPD.distances)
                    ...
                    && Arrays.deepEquals(this.startToProduct, otherPD.startToProduct) && this.spec.equals(otherPD.spec)
    ...
    private Route[][] buildDistanceMatrix(AntColonyOptimization aco) {
        ...
        for (int i = 0; i < numberOfProduct; i++) {
            for (int j = 0; j < numberOfProduct; j++) {
                ...
                System.out.println(productToProduct[i][j].getRoute().size());
            }
        }
    ...
    /**
     * Read a TSP problem specification based on a coordinate file and a product
     * file
     * 
